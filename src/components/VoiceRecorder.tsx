  ...
  return (
    <div className="bg-white/5 rounded-[2.5rem] p-8 border border-white/10 space-y-6">
      <div className="flex justify-between items-center">
        <DotMatrixText color="text-nothing-yellow">STRATEGIC_SPARK_CAPTURE</DotMatrixText>
        <div className="flex items-center gap-2 text-[10px] font-mono text-white/20">
          <Clock size={12} />
          <span className={cn(duration >= 25 ? "text-nothing-yellow" : "")}>{duration}s / 30s</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {transcript && (
          <div className="bg-black/20 border border-white/5 rounded-2xl p-4 space-y-2">
            <DotMatrixText color="text-nothing-yellow">VOICE_TO_TEXT_TRANSCRIPT</DotMatrixText>
        ...
        <div className="flex items-center justify-between bg-black/20 rounded-2xl p-4 border border-white/5">
          <div className="flex items-center gap-3">
            {isGated ? <Shield size={18} className="text-nothing-yellow" /> : <ShieldOff size={18} className="text-white/20" />}
          ...
          <button 
            ...
            className={cn(
              ...
              isGated ? "bg-nothing-yellow" : "bg-white/10"
            )}
          >
            <motion.div 
          ...
          <AnimatePresence mode="wait">
            {!isRecording && !audioBlob ? (
              <motion.button
                ...
                className="w-24 h-24 rounded-full bg-nothing-yellow flex items-center justify-center text-black shadow-2xl shadow-nothing-yellow/40 hover:scale-110 transition-transform"
              >
                <Mic size={32} />
              </motion.button>
            ) : isRecording ? (
              <motion.button
                ...
                className="w-24 h-24 rounded-full bg-black border-4 border-nothing-yellow flex items-center justify-center text-nothing-yellow shadow-2xl shadow-nothing-yellow/20"
              >
                <div className="w-8 h-8 bg-nothing-yellow rounded-sm animate-pulse" />
              </motion.button>
            ) : (
              <motion.div
                key="preview"
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center text-nothing-yellow">
                    <Volume2 size={32} />
                  </div>
                  <button onClick={handleSave} disabled={!title} className="w-12 h-12 rounded-full bg-nothing-yellow flex items-center justify-center text-black disabled:opacity-20">
                    <Save size={20} />
                  </button>
                </div>
                <DotMatrixText color="text-nothing-yellow">INSIGHT_CAPTURED</DotMatrixText>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="flex justify-center gap-1 h-8 items-end">
          {[...Array(12)].map((_, i) => (
            <motion.div
              ...
              className="w-1 bg-nothing-yellow rounded-full"
            />
          ))}
        </div>
      )}
